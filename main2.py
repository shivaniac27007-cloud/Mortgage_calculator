import streamlit as st          
import pandas as pd             
import matplotlib.pyplot as plt
import math                    

# App title
st.title("Mortgage Repayments Calculator")


# USER INPUT 

# Section heading
st.subheader("Input data")
# Create two columns 
col1, col2 = st.columns(2)#split data into 2 different columns

# Inputs in column 1
home_value = col1.number_input(
    "Home Value",               # Label shown to user
    min_value=0,                # Prevent negative values
    value=500000                # Default value
)

deposit = col1.number_input(
    "Deposit",
    min_value=0,
    value=100000
)

# Inputs in column 2
interest_rate = col2.number_input(
    "Interest Rate (in %)",
    min_value=0.0,
    value=5.5
)

loan_term = col2.number_input(
    "Loan Term (in years)",
    min_value=1,
    value=30
)


# LOAN CALCULATIONS

# Calculate the loan amount
loan_amount = home_value - deposit

# Convert annual interest rate to monthly
monthly_interest_rate = (interest_rate / 100) / 12

# Total number of monthly payments
number_of_payments = loan_term * 12

# Formula to calculate fixed monthly mortgage payment
monthly_payment = (
    loan_amount
    * (monthly_interest_rate * (1 + monthly_interest_rate) ** number_of_payments)
    / ((1 + monthly_interest_rate) ** number_of_payments - 1)
)


# DISPLAY REPAYMENT

# Calculate total amount paid and interest
total_payments = monthly_payment * number_of_payments
total_interest = total_payments - loan_amount

# Section heading
st.subheader("Repayments")

# Display key metrics in three columns
col1, col2, col3 = st.columns(3)

st.title("Home Loan EMI Calculator")

st.metric("Monthly EMI", f"₹{monthly_payment:,.2f}")
st.metric("Total Amount Payable", f"₹{total_payments:,.0f}")
st.metric("Total Interest Payable", f"₹{total_interest:,.0f}")

# PAYMENT SCHEDULE CALCULATION

# List to store payment details for each month
schedule = []

# Initial remaining loan balance
remaining_balance = loan_amount

# Loop through each monthly payment
for i in range(1, number_of_payments + 1):

    # Interest portion of the payment
    interest_payment = remaining_balance * monthly_interest_rate

    # Principal portion of the payment
    principal_payment = monthly_payment - interest_payment

    # Reduce remaining balance
    remaining_balance -= principal_payment

    # Calculate the loan year (1–30)
    year = math.ceil(i / 12)

    # Store the payment details
    schedule.append([
        i,                       # Month number
        monthly_payment,         # Total payment
        principal_payment,       # Principal part
        interest_payment,        # Interest part
        remaining_balance,       # Balance after payment
        year                     # Loan year
    ])

# Convert schedule list to DataFrame
df = pd.DataFrame(
    schedule,
    columns=[
        "Month",
        "Payment",
        "Principal",
        "Interest",
        "Remaining Balance",
        "Year"
    ],
)


# VISUALIZATION


# Section heading
st.subheader("Payment Schedule")

# Group by year and get minimum remaining balance per year
payments_df = df[["Year", "Remaining Balance"]].groupby("Year").min()

# Display line chart showing loan balance reducing over time
st.line_chart(payments_df)


